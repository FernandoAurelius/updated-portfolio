<template>
  <div class="max-w-7xl mx-auto border-gray-200 dark:border-gray-700 border-dashed border-l border-r">
    <div class="text-gray-800 dark:text-gray-200">
      <div class="px-4 py-4 max-w-5xl mx-auto sm:px-6 lg:px-8">
        <article data-aos="fade-up">
          <div class="space-y-9">
            <div
              class="py-0.5 border-t border-b border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div class="text-indigo-600 dark:text-indigo-500 font-bold">
                <nuxt-link class="hover:text-indigo-500 dark:hover:text-indigo-400" :to="localePath('/projects')">
                  {{ $t('projects.header') }}
                </nuxt-link>
                <span class="text-gray-400 dark:text-gray-700">/</span> {{ project.tech }}
              </div>
            </div>
            <div class="px-4 sm:px-0 lg:px-4 pb-4 bg-white dark:bg-gray-900 rounded-lg shadow-sm dark:shadow-none">
              <header class="py-4">
                <div class="space-y-1 text-center">
                  ...
                  <dl>
                    <div>
                      ...
                      <dd class="text-xs font-medium text-gray-500 dark:text-gray-400">
                        <a target="_blank" rel="noreferrer" :href="project.website">
                          <span
                            class="text-indigo-600 hover:text-indigo-500 dark:text-indigo-500 dark:hover:text-indigo-400">
                            {{ project.title }}
                          </span>
                        </a>
                  ...
                  <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div
                      class="px-3 text-xs flex flex-shrink-0 py-1 mt-1 mr-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-150"
                      v-for="(tag, index) in project.tags" :key="`tag-${index}`">
                      {{ tag }}
                    </div>
                  </div>
                </div>
